...
void StatisticsSampler::sample() {
    ...
    if(system->myid == 0) {
        kinetic_energy /= system->num_atoms_global;
        potential_energy /= system->num_atoms_global;

        cout << "Energy (Ek Ep Etot) = " << kinetic_energy << " " << potential_energy << " " << potential_energy + kinetic_energy << endl;
